########################################################################################################
####
#### Vectorizing the estimation the post-offer patient mortality
####
...
candidate_iter_vec <- function(dataset, 
                               pt_index, 
                               evaluated_t, 
                               allocation_sharing,
                               age_sharing,
  ...
  ########################################################################################################
  ####
  #### Estimating the cumulative incidence functions
  ####
  
  tmp_p_dd  <- tmp_cif$pstate[1:length(cif_ts), 1]
  tmp_p_ld  <- tmp_cif$pstate[1:length(cif_ts), 2]
  tmp_p_rem <- tmp_cif$pstate[1:length(cif_ts), 3]
  tmp_p_wlm <- tmp_cif$pstate[1:length(cif_ts), 4]
  
  p_dd  <- diff(c(0, tmp_p_dd))
  p_ld  <- diff(c(0, tmp_p_ld))
  p_rem <- diff(c(0, tmp_p_rem))
  p_wlm <- diff(c(0, tmp_p_wlm))
  
  
  ########################################################################################################
  ####
  #### Estimating the 'true' cumulative incidence functions
  ####

    
  tmp_p_o_dd <- sapply(cif_ts, function(x){
  ...
  tmp_p_o_ld <- sapply(cif_ts, function(x){
    CIF_Weibull_calc(true_dd_shape, true_dd_scale[pt_index], 
                     true_ld_shape, true_ld_scale, 
                     true_rem_shape, true_rem_scale[pt_index], 
                     true_wlm_shape, true_wlm_scale[pt_index], 
  ...
  tmp_p_o_rem <- sapply(cif_ts, function(x){
    CIF_Weibull_calc(true_dd_shape, true_dd_scale[pt_index], 
                     true_ld_shape, true_ld_scale, 
                     true_rem_shape, true_rem_scale[pt_index], 
                     true_wlm_shape, true_wlm_scale[pt_index], 
  ...
  tmp_p_o_wlm <- sapply(cif_ts, function(x){
    CIF_Weibull_calc(true_dd_shape, true_dd_scale[pt_index], 
                     true_ld_shape, true_ld_scale, 
                     true_rem_shape, true_rem_scale[pt_index], 
                     true_wlm_shape, true_wlm_scale[pt_index], 
  ...
  p_o_dd  <- diff(c(0, tmp_p_o_dd))
  p_o_ld  <- diff(c(0, tmp_p_o_ld))
  p_o_rem <- diff(c(0, tmp_p_o_rem))
  p_o_wlm <- diff(c(0, tmp_p_o_wlm))
  ...
  preds_all    <- sum(p_dd * (1 - dd_surv_preds) + p_ld * (1 - ld_surv_preds) + p_rem * (1 - rem_surv_preds) + p_wlm)
  preds_median <- sum(p_dd * (1 - dd_surv_preds_median) + p_ld * (1 - ld_surv_preds_median) + p_rem * (1 - rem_surv_preds_median) + p_wlm)
  preds_true   <- sum(p_o_dd * (1 - dd_surv_true) + p_o_ld * (1 - ld_surv_true) + p_o_rem * (1 - rem_surv_true) + p_o_wlm)
  
  
  cif_ests <- cbind(p_dd, p_o_dd, p_ld, p_o_ld, p_rem, p_o_rem, p_wlm, p_o_wlm)
